...
class ClassificationOutcome(NamedTuple):
    ...
    score: float | None
    signals: tuple[str, ...]
    cause: Literal["heuristic_scorer", "llm_classifier"]
...
class ComplexityRouter(CustomLogger):
    ...
    def classify(self, prompt: str, system_prompt: str | None = None) -> tuple[ComplexityTier, float, list[str]]:
        ...
        if reasoning_match_count >= 2:
            return ComplexityTier.REASONING, weighted_score, [*signals, "reasoning-override"]
    ...
    async def aclassify(
        ...
        if self.config.classifier_type != "llm" or self.config.classifier_llm_config is None:
            ...
            return ClassificationOutcome(tier=tier, score=score, signals=tuple(signals), cause="heuristic_scorer")
    ...
    async def _classify_and_route(
        ...
        if override is not None:
            ...
            verbose_router_logger.info(
                f"ComplexityRouter: routing decision cause={keyword_cause}, escalated={escalation_keyword is not None}, "
