...
def _build_reasoning_item(
    ...
    return {"id": item_id, "type": "reasoning", "encrypted_content": encrypted_content, "summary": summary}
...
class LiteLLMResponsesTransformationHandler(CompletionTransformationBridge):
    ...
    def _handle_raw_dict_response_item(self, item: dict[str, Any], index: int) -> tuple["Choices | None", int]:
        ...
        if item_type == "apply_patch_call":
            operation: Final = item.get("operation")
    ...
    def _convert_response_output_to_choices(
        ...
        pending_reasoning_items: list[_BuiltReasoningItem] = []
            ...
            if isinstance(item, ResponseReasoningItem):
                ...
                if raw_item.get("type") == "reasoning":
                    reasoning_item = _build_reasoning_item(
                        ...
                        summary_raw=raw_item.get("summary"),
                    )
                    pending_reasoning_items.append(reasoning_item)
                    reasoning_parts.extend(
                        str(summary.get("text") or "") for summary in reasoning_item["summary"] if summary.get("text")
                ...
                if handle_raw_dict_callback is None:
                    continue
                choice, next_index = handle_raw_dict_callback(item=raw_item, index=index)
        ...
        if accumulated_tool_calls:
            ...
            if last_message_choice is None:
                ...
                message: Final = last_message_choice.message
                message.tool_calls = normalized_tool_calls
                if message.content is None:
                    message.content = ""
                last_message_choice.finish_reason = "tool_calls"
                reasoning_content: Final = " ".join(reasoning_parts)
...
class OpenAiResponsesToChatCompletionStreamIterator(BaseModelResponseIterator):
    ...
    def translate_responses_chunk_to_openai_stream(
        ...
        *,
        ...
        if event_type == "response.created":
            ...
            arguments: Final = parsed_chunk.get("arguments")
            ...
            finish_reason: Final = (
                ...
                if event_type == "response.incomplete"
                    ...
                    "tool_calls" if has_function_calls or (saw_function_call and "response" in parsed_chunk) else "stop"
                )
            )
    ...
    def chunk_parser(self, chunk: dict[str, object]) -> "ModelResponseStream":
        ...
        if event_type == ResponsesAPIStreamEvents.OUTPUT_ITEM_ADDED.value:
            output_item: Final = chunk.get("item") or {}
            if isinstance(output_item, dict) and output_item.get("type") in ("function_call", "custom_tool_call"):
            ...
            ResponsesAPIStreamEvents.CUSTOM_TOOL_CALL_INPUT_DONE.value,
        ):
            self._saw_function_call = True
