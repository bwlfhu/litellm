...
def _sanitize_tool_use_id_content_block(block: object) -> tuple[object, bool, bool]:
    ...
    block_dict: Final = cast(dict[str, object], block)  # cast-ok: guarded dict from an untyped request payload
...
def _sanitize_tool_use_ids_in_anthropic_message(message: object) -> tuple[object, int, int]:
    if not isinstance(message, dict):
        return message, 0, 0
    message_dict: Final = cast(dict[str, object], message)  # cast-ok: guarded dict from an untyped message payload
    content_value: Final = message_dict.get("content")
    if not isinstance(content_value, list):
        return message_dict, 0, 0
    content: Final = cast(list[object], content_value)  # cast-ok: guarded list from an untyped message payload
    sanitized_blocks: Final = tuple(_sanitize_tool_use_id_content_block(block) for block in content)
    new_content: Final = [block for block, _, _ in sanitized_blocks]
    scanned_id_count: Final = sum(scanned for _, scanned, _ in sanitized_blocks)
    rewritten_id_count: Final = sum(rewritten for _, _, rewritten in sanitized_blocks)
    sanitized_message: Final = message_dict if rewritten_id_count == 0 else {**message_dict, "content": new_content}
...
def sanitize_tool_use_ids_in_anthropic_messages_with_stats(
    ...
    sanitized_messages: Final = tuple(_sanitize_tool_use_ids_in_anthropic_message(message) for message in messages)
    return (
