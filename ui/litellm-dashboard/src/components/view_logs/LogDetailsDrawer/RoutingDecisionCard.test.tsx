...
describe("RoutingDecisionCard", () => {
  ...
  it("renders nothing for a malformed decision with no cause", () => {
    const { container } = render(<RoutingDecisionCard decision={{ routed_model: "x" } as RoutingDecision} />);
    expect(container).toBeEmptyDOMElement();
  });
    ...
    render(
      <RoutingDecisionCard
        decision={{
          ...heuristic,
          score: 0.2,
          signals: ["reasoning (prove, step-by-step)", "reasoning-override"],
        }}
      />,
    );
  ...
  it("omits the escalation row when no keyword was involved", () => {
    render(<RoutingDecisionCard decision={heuristic} />);
    expect(screen.queryByText("Escalated")).not.toBeInTheDocument();
  });

  it("labels a session pin", () => {
    render(
      <RoutingDecisionCard
        decision={{
          router_model_name: "pinned-router",
          router_type: "complexity",
          routed_model: "claude-haiku",
          cause: "session_affinity_pin",
        }}
      />,
    );
    expect(screen.getByText("Pinned to session")).toBeInTheDocument();
  });
