...
class TestSSEKeepalive:
    ...
    async def test_disconnect_before_timer_still_returns_499_when_keepalive_enabled(self, monkeypatch):
    ...
    async def test_disconnect_after_timer_hands_back_task_then_closes_upstream(self, monkeypatch):
    ...
    async def test_early_consumer_exit_cancels_pending_and_closes_upstream(self, monkeypatch, caplog):
        monkeypatch.setattr(litellm, "sse_keepalive_interval_seconds", 1.0, raising=False)
        closed = asyncio.Event()
    ...
    async def test_raising_fetch_outcome_is_consumed_so_asyncio_stays_quiet(self, monkeypatch, caplog):
    ...
    async def test_teardown_awaits_cancelled_fetch_before_closing_upstream(self, monkeypatch, caplog):
        ...
        assert events == ["upstream_closed", "call_returned"], f"cleanup escaped __call__: {events}"
        assert "asynchronous generator is already running" not in caplog.text

    async def test_chunk_landing_during_disconnect_teardown_is_not_discarded(self, monkeypatch):
    ...
    async def test_disconnect_during_keepalive_window_completes_shielded_cleanup(self, monkeypatch):
    ...
    async def test_response_carries_pending_task_so_it_cannot_be_orphaned(self, monkeypatch):
