...
def test_reset_budget_for_keys_invalidates_redis_counter(reset_budget_job, mock_prisma_client, monkeypatch):
    ...
    counter_cache.user_api_key_cache.async_delete_cache.assert_awaited_once_with(
        key="sk-abc"
    )


def test_reset_budget_for_keys_preserves_concurrent_new_window_spend(
    ...
    counter_cache.in_memory_cache.set_cache.assert_any_call(
        key="spend:key:sk-race", value=7.5, ttl=60
    )
