...
def test_update_kwargs_with_deployment_syncs_custom_pricing_to_prebuilt_logger():
    ...
    router = litellm.Router(
        model_list=[
            {
                "model_name": "custom-anthropic-group",
                "litellm_params": {
                    "model": "anthropic/unmapped-custom-model",
                    "api_key": "fake-key",
                },
                "model_info": {
                    "id": "custom-anthropic-pricing-id",
                    "input_cost_per_token": 0.0003,
                    "output_cost_per_token": 0.0015,
                },
            },
    ...
    deployment = router.get_deployment_by_model_group_name(model_group_name="custom-anthropic-group")
    ...
    try:
        ...
        response = ModelResponse(
            ...
            model="unmapped-custom-model",
            usage={"prompt_tokens": 10, "completion_tokens": 5, "total_tokens": 15},
        )
        cost_payload = AnthropicPassthroughLoggingHandler._create_anthropic_response_logging_payload(
    ...
    assert logging_obj.get_router_model_id() == "custom-anthropic-pricing-id"
    assert cost_payload["response_cost"] == pytest.approx(0.0105)
