...
def test_streaming_content_block_classifier_prioritizes_reasoning_over_text():
    """A combined reasoning+text chunk must open a thinking block.

    The delta translator emits ``thinking_delta`` whenever reasoning content is
    present, even if the same upstream chunk also carries visible text. The
    classifier must therefore choose the same block type; otherwise Claude Code
    receives a thinking delta inside a text block.
    """
    choices = [
        StreamingChoices(
            finish_reason=None,
    ...
    assert block_type == "thinking"
    assert content_block["type"] == "thinking"
