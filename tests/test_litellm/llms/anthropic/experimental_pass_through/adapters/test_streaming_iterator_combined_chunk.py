...
def test_split_preserves_metadata_only_chunks():
    metadata_chunk = SimpleNamespace(choices=[])
    assert _CombinedChunkSplitter._split(metadata_chunk) == [metadata_chunk]


def test_split_preserves_chunks_without_delta():
    chunk = SimpleNamespace(choices=[SimpleNamespace(finish_reason=None, delta=None)])
    assert _CombinedChunkSplitter._split(chunk) == [chunk]


def test_split_clears_reasoning_and_thinking_on_finish_chunk():
    """Reasoning, visible text, and finish state become three ordered chunks."""
    ...
    reasoning_chunk, text_chunk, finish_chunk = _CombinedChunkSplitter._split(chunk)
    ...
    assert text_chunk.choices[0].delta.content == "hi"
    assert text_chunk.choices[0].delta.reasoning_content is None
    assert text_chunk.choices[0].delta.thinking_blocks is None
