...
class TestFunctionCallTransformation:
    ...
    def test_ensure_tool_results_preserves_cached_openai_object_tool_call(self):
        ...
        response_id = "response_cached_openai"
        cache_key = LiteLLMCompletionResponsesConfig._tool_call_cache_key(response_id, tool_call_id)
        ...
        try:
            ...
            TOOL_CALLS_CACHE.delete_cache(key=cache_key)
    ...
    def test_ensure_tool_results_preserves_cached_attr_object_tool_call(self):
        ...
        response_id = "response_cached_attr"
        cache_key = LiteLLMCompletionResponsesConfig._tool_call_cache_key(response_id, tool_call_id)
        TOOL_CALLS_CACHE.set_cache(
            key=cache_key,
...
class TestToolTransformation:
    ...
    def test_namespace_tool_name_map_accepts_unique_unqualified_tool_names(self):
        ...
        result = LiteLLMCompletionResponsesConfig._namespace_tool_name_map([namespace_tool])

        assert result["collaboration__wait_agent"] == ("collaboration", "wait_agent", "function")
        assert result["wait_agent"] == ("collaboration", "wait_agent", "function")
    ...
    def test_namespace_tool_name_map_drops_ambiguous_unqualified_names(self):
        ...
        result = LiteLLMCompletionResponsesConfig._namespace_tool_name_map(tools)

        assert result["alpha__run"] == ("alpha", "run", "function")
        assert result["beta__run"] == ("beta", "run", "function")
        assert result["gamma__run"] == ("gamma", "run", "function")
    ...
    def test_namespace_tool_name_map_drops_top_level_function_collision(self):
        ...
        assert result["admin__run"] == ("admin", "run", "function")
        assert "run" not in result
    ...
    def test_restore_namespace_tool_name_leaves_unknown_tool_unchanged(self):
        tool_name, namespace, tool_type = LiteLLMCompletionResponsesConfig._restore_namespace_tool_name(
            "mcp__node_repl",
            {},
        )
    ...
    def test_transform_nested_namespace_ignores_non_function_subtools(self):
        ...
        with pytest.raises(litellm.BadRequestError, match="contains an invalid tool"):
            LiteLLMCompletionResponsesConfig.transform_responses_api_tools_to_chat_completion_tools(
                tools=[namespace_tool]
            )
...
class TestEnsureOutputItemContentPartAdded:
    ...
    def _make_iterator(self):
        ...
        wrapper.logging_obj = None
        return LiteLLMCompletionStreamingIterator(
            model="test-model", litellm_custom_stream_wrapper=wrapper, request_input="test", responses_api_request={}
        )

    def _make_text_chunk(self):
        from litellm.types.utils import ModelResponseStream

        return ModelResponseStream(choices=[{"index": 0, "delta": {"content": "text"}}])

    def _make_reasoning_chunk(self):
        from litellm.types.utils import ModelResponseStream

        return ModelResponseStream(choices=[{"index": 0, "delta": {"reasoning_content": "thinking..."}}])
    ...
    def test_streaming_namespace_tool_calls_restore_responses_namespace(self):
        ...
        iterator._namespace_tool_names = LiteLLMCompletionResponsesConfig._namespace_tool_name_map(
    ...
    def test_streaming_namespace_map_is_built_once(self):
        ...
        with patch.object(
            LiteLLMCompletionResponsesConfig,
            "_namespace_tool_name_map",
            wraps=LiteLLMCompletionResponsesConfig._namespace_tool_name_map,
        ) as namespace_map:
            iterator = LiteLLMCompletionStreamingIterator(
                model="test-model",
                litellm_custom_stream_wrapper=mock_stream_wrapper,
...
@pytest.mark.parametrize("first_text,with_tool", [(True, False), (True, True), (False, True)])
@pytest.mark.parametrize("finish_reason", ["stop", "length"])
async def test_multistep_stream_items_match_completed_snapshot(async_mode, first_text, with_tool, finish_reason):
    from unittest.mock import MagicMock
    ...
    chunks = [
        ...
        *(
        ...
        ModelResponseStream(
            id="step-first",
            model="gpt-5.5",
            choices=[{"index": 0, "delta": {}, "finish_reason": "tool_calls" if with_tool else "stop"}],
        ),
        ModelResponseStream(
            id="usage-first",
            model="gpt-5.5",
    ...
    wrapper = MagicMock(spec=litellm.CustomStreamWrapper)
    wrapper.logging_obj = None
    wrapper.__next__.side_effect = iter(chunks)
    wrapper.__anext__.side_effect = chunks
    iterator = LiteLLMCompletionStreamingIterator(
        model="gpt-5.5", litellm_custom_stream_wrapper=wrapper, request_input="test", responses_api_request={}
    )
    ...
    expected_count = 1 + int(first_text) + int(with_tool)
    assert len(added) == len(done) == len(completed[0].response.output) == expected_count
    assert [event.output_index for event in added] == list(range(expected_count))
    assert len({event.item.id for event in added}) == expected_count
    assert {event.item.id for event in added} == {event.item.id for event in done}
    for event in done:
        assert completed[0].response.output[event.output_index].model_dump(exclude_none=True) == event.item.model_dump(
            exclude_none=True
        )
    messages = [item for item in completed[0].response.output if item.type == "message"]
    assert [item.content[0]["text"] for item in messages] == (["First", "Second"] if first_text else ["Second"])
    if first_text:
        assert messages[0].status == "completed"
    assert messages[-1].status == ("incomplete" if finish_reason == "length" else "completed")
    assert completed[0].response.status == ("incomplete" if finish_reason == "length" else "completed")
    for event in events:
        if event.type in {"response.output_text.delta", "response.output_text.done", "response.content_part.done"}:
            assert completed[0].response.output[event.output_index].id == event.item_id
            assert isinstance(event.sequence_number, int)
    if with_tool:
        assert completed[0].response.output[int(first_text)].arguments == '{"id":1}'


@pytest.mark.asyncio
@pytest.mark.parametrize("async_mode", [False, True])
@pytest.mark.parametrize("combined_chunk", [False, True])
async def test_multistep_reasoning_and_reused_tool_indexes(async_mode, combined_chunk):
    from unittest.mock import MagicMock
    ...
    chunks = []
    for step in (1, 2):
        content = f"Answer {step}"
        reasoning = f"Reason {step}"
        tool_delta = [
            {
                "id": f"call-{step}",
                "index": 0,
                "type": "function",
                "function": {"name": f"lookup_{step}", "arguments": '{"id":'},
                "provider_specific_fields": {"thought_signature": f"tool-signature-{step}"},
            }
        ]
        deltas = (
            [{"role": "assistant", "reasoning_content": reasoning, "content": content, "tool_calls": tool_delta}]
            if combined_chunk
            else [
        ...
        chunks.extend(
            ModelResponseStream(id=f"step-{step}", model="gpt-5.5", choices=[{"index": 0, "delta": delta}])
            for delta in [
                *deltas,
                {"tool_calls": [{"index": 0, "function": {"arguments": f"{step}}}"}}]},
            ]
        )
        chunks.append(
            ModelResponseStream(
    ...
    assert [event.item.type for event in added] == ["reasoning", "message", "function_call"] * 2
    assert [event.output_index for event in added] == list(range(6))
    assert len({event.item.id for event in added}) == len(done) == len(completed.output) == 6
    for event in done:
        assert completed.output[event.output_index].model_dump(exclude_none=True) == event.item.model_dump(
            exclude_none=True
        )
    assert [item.summary[0]["text"] for item in completed.output if item.type == "reasoning"] == [
        "Reason 1",
        "Reason 2",
    ]
    for step in (1, 2):
        tool = completed.output[step * 3 - 1]
        assert tool.name == f"lookup_{step}"
        assert tool.arguments == f'{{"id":{step}}}'
        assert tool.provider_specific_fields == {"thought_signature": f"tool-signature-{step}"}
        deltas = [
            event.delta
            for event in events
            if event.type == "response.function_call_arguments.delta" and event.item_id == f"call-{step}"
        ]
        assert "".join(deltas) == tool.arguments
        cached = TOOL_CALLS_CACHE.get_cache(
            LiteLLMCompletionResponsesConfig._tool_call_cache_key(iterator._cached_response_id, f"call-{step}")
        )
        assert cached.function.arguments == tool.arguments
        assert cached.function.name == tool.name
    numbered = [event.sequence_number for event in events if getattr(event, "sequence_number", None) is not None]
    assert numbered == sorted(set(numbered))


@pytest.mark.asyncio
@pytest.mark.parametrize("async_mode", [False, True])
@pytest.mark.parametrize("with_finish_chunk", [False, True])
@pytest.mark.parametrize("with_reasoning_field", [False, True])
async def test_reasoning_only_stream_preserves_signed_history(async_mode, with_finish_chunk, with_reasoning_field):
    from unittest.mock import MagicMock
            ...
            choices=[
                {
                    ...
                    "delta": {
                        "role": "assistant",
                        "reasoning_content": "Thought" if with_reasoning_field else None,
                        "thinking_blocks": [{"type": "thinking", "thinking": "Thought", "signature": "signed-history"}],
                    },
                }
            ],
        )
    ]
    if with_finish_chunk:
        chunks.append(
            ModelResponseStream(
                id="reasoning-step", model="gpt-5.5", choices=[{"index": 0, "delta": {}, "finish_reason": "stop"}]
            )
        )
    wrapper = MagicMock(spec=litellm.CustomStreamWrapper)
    wrapper.logging_obj = None
    ...
    completed = events[-1].response
    assert len(added) == len(done) == len(completed.output) == 1
    assert added[0].item.id == done[0].item.id == completed.output[0].id
    assert completed.output[0].type == "reasoning"
    assert completed.output[0].summary == [{"type": "summary_text", "text": "Thought"}]
    assert "signed-history" in completed.output[0].encrypted_content
...
class TestStreamingSnapshotItemIds:
    ...
    def test_incremental_item_id_uses_msg_prefix(self):
        ...
        event = iterator._transform_chat_completion_chunk_to_response_api_chunk(self._make_chunk("apple"))
    ...
    def test_completed_snapshot_reuses_streamed_item_id(self):
        ...
        streamed_event = iterator._transform_chat_completion_chunk_to_response_api_chunk(self._make_chunk("apple"))
        assert streamed_event is not None

        completed_event = iterator._emit_response_completed_event(_bridged_chat_completion_response())
    ...
    def test_completed_snapshot_item_id_is_replayable(self):
        iterator = self._make_iterator()
        iterator._transform_chat_completion_chunk_to_response_api_chunk(self._make_chunk("apple"))
    ...
    def _reasoning_chat_completion_response(self):
        message = Message(role="assistant", content="apple")
        message.reasoning_content = "thinking about fruit"
        return _bridged_chat_completion_response(choices=[Choices(index=0, finish_reason="stop", message=message)])
    ...
    def test_reasoning_delta_events_share_one_item_id(self):
        ...
        first = iterator._transform_chat_completion_chunk_to_response_api_chunk(self._make_reasoning_chunk("thinking "))
    ...
    def test_completed_snapshot_reuses_streamed_reasoning_item_id(self):
        ...
        completed_event = iterator._emit_response_completed_event(self._reasoning_chat_completion_response())
