...
def test_done_only_arguments_survive_chatgpt_normalizer_and_iterator_state_is_isolated():
    ...
    second_chunks = [
        ...
        second.chunk_parser({"type": "response.completed", "response": {"status": "completed"}}),
    ]
...
def test_reasoning_on_both_sides_of_message_is_not_dropped():
    ...
    assert [item["encrypted_content"] for item in choices[0].message.reasoning_items] == ["encrypted-1", "encrypted-2"]
...
def test_tool_only_and_message_only_response_shapes_are_unchanged():
    tool_choices = LiteLLMResponsesTransformationHandler._convert_response_output_to_choices([_batch_d_function_call()])
